...
using Indt.Unit.Domain.Entities.TestData;
using Xunit;

namespace Indt.Unit.Domain.Entities;
...
public class UserTests
{
    ...
    public void Given_SuspendedUser_When_Activated_Then_StatusShouldBeActive()
    {
        ...
        user.Status = ProposalStatus.Suspended;

        // Act
        user.Activate();

        // Assert
        Assert.Equal(ProposalStatus.Active, user.Status);
    }
    ...
    public void Given_ActiveUser_When_Suspended_Then_StatusShouldBeSuspended()
    {
        // Arrange
        var user = UserTestData.GenerateValidUser();
        user.Status = ProposalStatus.Active;

        // Act
        user.Suspend();

        // Assert
        Assert.Equal(ProposalStatus.Suspended, user.Status);
    }
    ...
    public void Given_InvalidUserData_When_Validated_Then_ShouldReturnInvalid()
    {
        ...
        var user = new User
        {
            ...
            Status = ProposalStatus.Unknown, // Invalid: cannot be Unknown
