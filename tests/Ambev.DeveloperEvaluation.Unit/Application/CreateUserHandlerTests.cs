...
using Indt.Unit.Application.TestData;
using NSubstitute;
using Security;
using Xunit;

namespace Indt.Unit.Application;

/// <summary>
/// Contains unit tests for the <see cref="CreatePropostaHandler"/> class.
/// </summary>
public class CreateUserHandlerTests
{
    private readonly IPropostaRepository _userRepository;
    private readonly IMapper _mapper;
    private readonly IPasswordHasher _passwordHasher;
    private readonly CreatePropostaHandler _handler;
    ...
    {
        _userRepository = Substitute.For<IPropostaRepository>();
        _mapper = Substitute.For<IMapper>();
        _passwordHasher = Substitute.For<IPasswordHasher>();
        _handler = new CreateUserHandler(_userRepository, _mapper, _passwordHasher, null);
    }
    ...
    public async Task Handle_ValidRequest_ReturnsSuccessResponse()
    {
        ...
        var user = new Proposal.Domain.Entities.Proposta
        {
            Id = Guid.NewGuid(),
        ...
        _mapper.Map<Proposal>(command).Returns(user);
        _mapper.Map<CreatePropostaResult>(user).Returns(result);

        _userRepository.CreateAsync(Arg.Any<Proposal>(), Arg.Any<CancellationToken>())
            .Returns(user);
        ...
        await _userRepository.Received(1).CreateAsync(Arg.Any<Proposal>(), Arg.Any<CancellationToken>());
    }
    ...
    {
        // Given
        var command = new CreatePropostaCommand(); // Empty command will fail validation
    ...
    public async Task Handle_ValidRequest_HashesPassword()
    {
        ...
        await _userRepository.Received(1).CreateAsync(
            Arg.Is<Proposal>(u => u.Password == hashedPassword),
    ...
    public async Task Handle_ValidRequest_MapsCommandToUser()
    {
        ...
        // Then
        _mapper.Received(1).Map<Proposal>(Arg.Is<CreatePropostaCommand>(c =>
